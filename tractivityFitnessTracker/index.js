...
app.get('/all', isAuthenticated, async function(request, response, next) {
  ...
  let results = await dbo.get_all(request.id)
...
app.get('/auth/accepted',
	...
	function (req, res) {
      ...
      // tell browser to get the hidden main page of the app
	    res.redirect('/index.html'); //	    res.redirect('/hello.html');
	});
...
//Prevents unlogged user from accessing index.html
app.get('/index.html', isAuthenticated); 
...
async function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    let previous = await dbo.check_profile(userid)
    if (!previous) {
      var name = profile.name["givenName"];

      await dbo.add_profile(userid, name)
    }
...
app.get('/name', isAuthenticated, async function(request, response, next) {
  ...
  try {
  let name = await dbo.get_name(userid)
  response.json({ username : name });
  } catch {
    console.log("Name was not received: " + request)
  }
});

app.post('/store', isAuthenticated, async function(request, response, next) { //should add isAuthenticated here but broken
  ...
  let activity = act.Activity(request.body, userid)
...
app.get('/reminder',  async function(request, response, next) {
  ...
  let result = await dbo.get_most_recent_planned_activity_in_range(0, currTime, request.user)
  await dbo.delete_past_activities_in_range(0, currTime, request.user);
...
app.get('/week',  isAuthenticated, async function(request, response, next) { //should add isAuthenticated here but breaks
  //console.log("Server recieved a post request at", request.url);
  ...
  if (activity === undefined) {
    let result = await dbo.get_most_recent_entry(request.user)
    try {
  ...
  let max = date
  let result = await dbo.get_similar_activities_in_range(activity, min, max, request.user)
  ...
  console.log("This is data.reverse: " + data.reverse())
