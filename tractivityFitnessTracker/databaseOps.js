...
const insertDB = "insert into ActivityTable (activity, date, amount, google_id) values (?,?,?,?)"
const getOneDB = "select * from ActivityTable where activity = ? and date = ? and google_id = ?";
const allDB = "select * from ActivityTable where activity = ?";
const deletePrevPlannedDB = "DELETE FROM ActivityTable WHERE amount < 0 and date BETWEEN ? and ? and google_id = ?";
const getMostRecentPrevPlannedDB = "SELECT rowIdNum, activity, MAX(date), amount FROM ActivityTable WHERE amount <= 0 and date BETWEEN ? and ? and google_id = ?";
const getMostRecentDB = "SELECT MAX(rowIdNum), activity, date, amount FROM ActivityTable WHERE google_id = ?";
const getPastWeekByActivityDB = "SELECT * FROM ActivityTable WHERE activity = ? and date BETWEEN ? and ? and google_id = ? ORDER BY date ASC";
...
async function testDB () {
  ...
  let startDate = newDate.getTime() - 20 * MS_IN_DAY;
  ...
  for(const entry of dbData) {
    for(let i = 0 ; i < entry.data.length; i++) {
      await db.run(insertDB,[entry.type, entry.start + i * MS_IN_DAY, entry.data[i]], "0000000000000000");
    }
  }
  

  await db.run(insertDB,["yoga", planDate2, -1, "0000000000000000"]);
  await db.run(insertDB,["yoga", planDate3, -1, "0000000000000000"]);
  await db.run(insertDB,["run", planDate2, -1, "0000000000000000"]);

  // some examples of getting data out of database
  
  // look at the item we just inserted
  let result = await db.get(getOneDB,["run",startDate, "0000000000000000"]);
...
async function add_profile(id, name) {

  const addProfile = "insert into Profile (google_id, first_name) values (?,?)";

  try {
    await db.run(addProfile, [id, name] );
  } catch (error) {
    console.log("error", error)
  }
}

async function check_profile(id) {
  const checkProfile = "select count(*) from Profile where google_id = ?";

  var found = 0;

  try {
    found = await db.run(checkProfile, [id]);
  }
  catch (error) {
    console.log("error", error)
    return false;
...
async function get_name(id) {
  const getName = "select * from Profile where google_id = ?"

  try {
    var name = await db.get(getName, [id]);
 ...
 * @param {number} max - ms since 1970
 */
async function delete_past_activities_in_range(min, max, id) {
  try {
    await db.run(deletePrevPlannedDB, [min, max, id]);
  }
  catch (error) {
    console.log(error);
  }
 ...
 */
async function newUTCTime() {
  let gmtDate = new Date()
  return (new Date(gmtDate.toLocaleDateString())).getTime()
}

async function randomNumber(min, max, round = true) { 
...
async function get_all(id) {
  try {
    let results = await db.all("select * from ActivityTable WHERE google_id = ?", [id]);
    return results;
  } 
  catch (error) {
    console.log(error);
